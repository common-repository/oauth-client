	...
	<?php
        if ( $currenttab == 'licensing' || ! get_option( 'mo_oauth_client_show_mo_server_message' ) ) {
            ...
            <form name="f" method="post" action="" id="mo_oauth_client_mo_server_form">
				<?php wp_nonce_field('mo_oauth_client_mo_server_message','mo_oauth_client_mo_server_message_field'); ?>
                ...
                <div class="notice notice-info" style="padding-right: 38px;position: relative;">
                    <h4 style="font-size: 13px;margin: 1.33em 0px;font-weight: 600;">Looking for a User Storage/OAuth Server? We have a B2C Service(Cloud IDP) which can scale to hundreds of millions of consumer identities. You can <a href="https://idp.miniorange.com/b2c-pricing" rel="noopener" target="_blank">click here</a> to find more about it.</h4>
...
function mo_oauth_show_customer_info_page() {
	...
	<div class="mo_table_layout" >
		...
		<tr>
			...
			<td style="width:55%; padding: 10px;"><?php echo esc_html(get_option( 'mo_oauth_admin_customer_key' )) ?></td>
		</tr>
		</table>
		<br /><br />
	...
	<form name="f1" method="post" action="" id="mo_oauth_goto_login_form">
		<?php wp_nonce_field('change_miniorange','change_miniorange_field'); ?>
	...
	<a href="<?php echo esc_url(add_query_arg( array( 'tab' => 'licensing' ), htmlentities( $_SERVER['REQUEST_URI'] ) )); ?>"><input type="button" class="button button-primary button-large" value="Check Licensing Plans"/></a>
	</td>
	</tr>
	</table>
...
function mo_oauth_show_new_registration_page() {
	...
	?>
		...
		<form name="f" method="post" action="">
			<?php wp_nonce_field('mo_oauth_register_customer','mo_oauth_register_customer_field'); ?>
                ...
                <!-- <p>Please enter a valid Email ID that you have access to. You will be able to move forward after verifying an OTP that we will be sending to this email.
                	...
                	<table class="mo_settings_table">
                		<tr>
                			...
                			<td><input class="mo_table_textbox" type="email" name="email" required placeholder="person@example.com" value="<?php echo esc_attr(get_option('mo_oauth_admin_email'));?>" />
                			</td>
                			    </tr>
                					<tr class="hidden">
                						<td><b><font color="#FF0000">*</font>Website/Company Name:</b></td>
                						<td><input class="mo_table_textbox" type="text" name="company"	required placeholder="Enter website or company name"
                					...
                					<tr  class="hidden">
                						...
                						<tr  class="hidden">
                							<td><b>&nbsp;&nbsp;Phone number :</b></td>
                							 <td><input class="mo_table_textbox" type="text" name="phone" pattern="[\+]?([0-9]{1,4})?\s?([0-9]{7,12})?" id="phone" title="Phone with country code eg. +1xxxxxxxxxx" placeholder="Phone with country code eg. +1xxxxxxxxxx" value="<?php echo esc_attr(get_option('mo_oauth_admin_phone'));?>" />
...
function mo_oauth_show_verify_password_page() {
	?>
		...
		<form name="f" method="post" action="">
			<?php wp_nonce_field('mo_oauth_verify_customer','mo_oauth_verify_customer_field'); ?>
						...
						<tr>
							...
							<td><input type="submit" name="submit"
								...
								<form id="mo_oauth_change_email_form" method="post" action="">
									<?php wp_nonce_field('mo_oauth_change_email','mo_oauth_change_email_field'); ?>
		...
		<form name="f" method="post" action="" id="mo_oauth_forgotpassword_form">
			<?php wp_nonce_field('mo_oauth_forgot_password_form_option','mo_oauth_forgot_password_form_option_field'); ?>	
...
function mo_oauth_sign_in_settings(){
	...
	<div class="mo_table_layout ">
		...
		<?php wp_nonce_field('mo_oauth_client_grant_type','mo_oauth_client_grant_type_field'); ?>
...
function mo__oauth_show_bfs_note(){
	?>
	<form name="f" method="post" action="" id="mo_oauth_client_bfs_note_form">
		<?php wp_nonce_field('mo_oauth_client_bfs_note_message','mo_oauth_client_bfs_note_message_field'); ?>
...
function mo_oauth_client_menu($currenttab){
	...
	<div class="wrap">
		<div><img style="float:left;" src="<?php echo esc_url(plugin_dir_url( __FILE__ ));?>/images/logo.png"></div>
		<h1>OAuth Client</h1>
	</div>
...
<div class="wrap">
			...
			<div style="float:left;">
				<a  class="add-new-h2 add-new-hover" style="font-size: 16px; color: #000;" href="<?php echo esc_url(add_query_arg( array( 'tab' => 'config' ), htmlentities( $_SERVER['REQUEST_URI'] ) ) ); ?>"><span class="dashicons dashicons-arrow-left-alt" style="vertical-align: bottom;"></span> Back To Plugin Configuration</a>
			</div>
	...
	<div class="tab-pane active text-center" id="cloud">
		...
		<div class="cd-pricing-container cd-has-margins"><br>
			...
			<input type="hidden" value="<?php mo_oauth_is_customer_registered();?>" id="mo_customer_registered">
			<ul class="cd-pricing-list cd-bounce-invert" >
				<li>

					<ul class="cd-pricing-wrapper">
						<li data-type="singlesite" class="mosslp is-visible" style="border: <?php echo esc_attr($sssborder); ?>">
							...
							<header class="cd-pricing-header">
								...
								<div class="cd-price" >
									...
									<span class="cd-value">299*</span></span>
							...
							<div class="cd-pricing-body">
								<ul class="cd-pricing-features">
									<li>1 OAuth / OIDC provider support</li>
									...
									<li>Login Widget</li>
									<li style="padding-bottom:8%!important;">Authorization Code Grant</li>
				...
				<li class="cd-popular">
					<ul class="cd-pricing-wrapper">
						<li data-type="singlesite" class="mosslp is-visible" style="border: <?php echo esc_attr($sspborder); ?>">
							...
							<div class="cd-pricing-body">
								<ul class="cd-pricing-features">
									...
									<li>Advanced Attribute Mapping (Username, FirstName, LastName, Email, Group Name)</li>
									<li>Login Widget</li>
									<li>Authorization Code Grant, Password Grant, Implicit Grant, Refresh token Grant <br> <br></li>
									<li>Login using link / shortcode</li>
									<li>Custom login buttons and CSS</li>
									<li>Custom Redirect URL after login and logout</li>
				...
				<li>
					<ul class="cd-pricing-wrapper">
						<li data-type="singlesite" class="mosslp is-visible" style="border: <?php echo esc_attr($sseborder); ?>">
							...
							<div class="cd-pricing-body">
								<ul class="cd-pricing-features ">
								<li>Unlimited OAuth / OIDC provider support</li>
									<li>Auto Create Users<br>(Unlimited Users)</li>
									<li>Account Linking</li>
									<li>Auto fill OAuth servers configuration</li>
									<li>Advanced Attribute Mapping (Username, FirstName, LastName, Email, Group Name)</li>
		...
		<div style="text-align:left; font-size:12px; padding-left:30px; padding-right:30px;">
			...
			resolved. Please email us at info@xecurify.com for any queries regarding the return policy.
	...
	<a  id="mobacktoaccountsetup" style="display:none;" href="<?php echo esc_url(add_query_arg( array( 'tab' => 'login' ), htmlentities( $_SERVER['REQUEST_URI'] ) ) );; ?>">Back</a>
	<form style="display:none;" id="loginform"
				action="<?php echo esc_attr(get_option( 'host_name' ) . '/moas/login');; ?>"
				target="_blank" method="post">
		<?php wp_nonce_field('mo_oauth_client_loginform','mo_oauth_client_loginform_field'); ?>
...
function mo_oauth_app_customization(){
	...
	<div id="mo_oauth_customiztion" class="mo_table_layout mo_oauth_app_customization <?php echo esc_attr($cclass); ?>">
	<form id="form-common" name="form-common" method="post" action="admin.php?page=mo_oauth_settings&tab=customization">
	<?php wp_nonce_field('mo_oauth_app_customization','mo_oauth_app_customization_field'); ?>
		...
		<table class="mo_settings_table">
			<tr>
				...
				<td><input type="text" id="mo_oauth_icon_width" name="mo_oauth_icon_width" value="<?php echo esc_attr(get_option('mo_oauth_icon_width'));?>"> e.g. 200px or 100%</td>
			</tr>
			<tr>
				<td><strong>Icon Height:</strong></td>
				<td><input  type="text" id="mo_oauth_icon_height" name="mo_oauth_icon_height" value="<?php echo esc_attr(get_option('mo_oauth_icon_height'));?>"> e.g. 50px or auto</td>
			</tr>
			<tr>
				<td><strong>Icon Margins:</strong></td>
				<td><input  type="text" id="mo_oauth_icon_margin" name="mo_oauth_icon_margin" value="<?php echo esc_attr(get_option('mo_oauth_icon_margin'));?>"> e.g. 2px 0px or auto</td>
			</tr>
			<tr>
				<td><strong>Custom CSS:</strong></td>
				<td><textarea type="text" id="mo_oauth_icon_configure_css" style="resize: vertical; width:400px; height:180px;  margin:5% auto;" rows="6" name="mo_oauth_icon_configure_css"><?php echo esc_attr(rtrim(trim(format_custom_css_value( $custom_css )),';'));?></textarea><br/><b>Example CSS:</b> 
 ...
 .custom_logo{
	 ...
	 color:#fff;
			...
			<tr>
				...
				<td><input type="text" style="resize: vertical; width:200px; height:30px;  margin:5% auto;" placeholder ="Howdy ,##user##" id="mo_oauth_custom_logout_text" name="mo_oauth_custom_logout_text" value="<?php echo esc_attr(get_option('mo_oauth_custom_logout_text'));?>"><b>##user## is replaced by Username</b></td>
			</tr>
			<tr>
				<td>&nbsp;</td>
...
function mo_oauth_apps_config() {
	...
	<?php
		...
		if(isset($_GET['action']) && $_GET['action']=='delete'){
			if(isset($_GET['app']) && check_admin_referer('mo_oauth_delete_'.sanitize_text_field($_GET['app']) ))
				mo_oauth_delete_app(sanitize_text_field($_GET['app']));
		} 
		...
		if(isset($_GET['action']) && $_GET['action']=='add'){
			...
			if(isset($_GET['app']))
				update_app(sanitize_text_field($_GET['app']));
		}
		else if(get_option('mo_oauth_apps_list'))
		{
			...
			foreach($appslist as $key => $app){
				$delete_url = wp_nonce_url( "admin.php?page=mo_oauth_settings&action=delete&app=".esc_html($key) , 'mo_oauth_delete_'.esc_html($key));
				echo "<tr><td>".esc_html($key)."</td><td><a href='admin.php?page=mo_oauth_settings&action=update&app=".esc_html($key)."'>Edit Application</a> | <a href='admin.php?page=mo_oauth_settings&action=update&app=".esc_html($key)."#attribute-mapping'>Attribute Mapping</a> | <a href='admin.php?page=mo_oauth_settings&action=update&app=".esc_html($key)."#role-mapping'>Role Mapping</a> | <a href='".esc_url($delete_url) ."' onclick =\"return confirm('Are you sure you want to delete this item?');\">Delete</a> | <a href='admin.php?page=mo_oauth_settings&action=update&app=".esc_html($key)."#howtoconfigure'>How to Configure?</a></td></tr>";
			}
			echo "</table>";
			echo "<br><br>";
...
function add_app(){
	...
	?>
			...
			function selectapp() {
				...
				if(appname=="google"){
				 ...
				 if(appname=="eveonline") { 
					...
					jQuery("#callbackurl").val("<?php echo esc_url(site_url());?>");
		...
		<?php wp_nonce_field('mo_oauth_add_app','mo_oauth_add_app_field'); ?>
		...
		<table class="mo_settings_table">
			...
			<td><input class="mo_table_textbox" id="callbackurl"  type="text" readonly="true" value='<?php echo esc_url(site_url());?>'></td>
			</tr>
			<tr  style="display:none" id="mo_oauth_custom_app_name_div">
				<td><strong><font color="#FF0000">*</font>Custom App Name:</strong></td>
				<td><input class="mo_table_textbox" type="text" id="mo_oauth_custom_app_name" name="mo_oauth_custom_app_name" value="" pattern="^[a-zA-Z0-9]+( [a-zA-Z0-9\s]+)*$" title="Please do not add any special characters."></td>
			</tr>
			...
			<tr>
				...
				<td><input class="mo_table_textbox" required="" type="password"  name="mo_oauth_client_secret" value=""></td>
			</tr>
			<tr>
			...
			<tr>
				<td><strong><?php esc_html_e('login button:','oauth-client')?></strong></td>
				<td><div style="padding:5px;"></div><input type="checkbox" name="mo_oauth_show_on_login_page" value ="1" checked/><?php esc_html_e('Show on login page','oauth-client')?></td>
			</tr>
...
function update_app($appname){
	...
	?>
		...
		<div id="toggle2" class="panel_toggle">
			<h3>Update Application : <?php echo esc_html($currentappname);?></h3>
		</div>
		<form id="form-common" name="form-common" method="post" action="admin.php?page=mo_oauth_settings">
		<?php wp_nonce_field('mo_oauth_add_app','mo_oauth_add_app_field'); ?>
		...
		<table class="mo_settings_table">
			...
			<td>
				<input class="mo_table_textbox" required="" type="hidden" name="mo_oauth_app_name" value="<?php echo esc_attr($currentappname);?>">
				<input class="mo_table_textbox" required="" type="hidden" name="mo_oauth_custom_app_name" value="<?php echo esc_attr($currentappname);?>">
				<?php echo esc_attr($currentappname);?><br><br>
			</td>
			</tr>
			<tr><td><strong>Redirect / Callback URL</strong></td>
			<td><input class="mo_table_textbox"  type="text" readonly="true" value='<?php echo esc_attr($currentapp['redirecturi']);?>'></td>
			</tr>
			
			<tr>
				<td><strong>Display App Name:</strong><br>&emsp;<font color="#FF0000"><small>[STANDARD]</small></font></td>
				<td><input disabled class="mo_table_textbox" type="text" name="mo_oauth_display_app_name" value="<?php echo isset($currentapp['displayappname']) ? esc_attr($currentapp['displayappname']) : '';?>"></td>
			</tr>
			...
			<tr>
				<td><strong>Scope:</strong></td>
				<td><input class="mo_table_textbox" type="text" name="mo_oauth_scope" pattern="[a-zA-Z0-9\s]+" title="Please do not add any special characters." value="<?php echo esc_attr($currentapp['scope']);?>"></td>
			</tr>
			<?php 
				if($is_eveonline){ 
					...
					$displaystyle= "";
			?>
			<tr  id="mo_oauth_authorizeurl_div" style="<?php echo esc_attr($displaystyle);?>">
				<td><strong><font color="#FF0000">*</font>Authorize Endpoint:</strong></td>
			...
			<tr id="mo_oauth_accesstokenurl_div" style="<?php echo esc_attr($displaystyle);?>">
				<td><strong><font color="#FF0000">*</font>Access Token Endpoint:</strong></td>
			...
			<tr id="mo_oauth_resourceownerdetailsurl_div" style="<?php echo esc_attr($displaystyle);?>">
				<td><strong><font color="#FF0000">*</font>Get User Info Endpoint:</strong></td>
				<td><input class="mo_table_textbox" required="" type="text" id="mo_oauth_resourceownerdetailsurl" name="mo_oauth_resourceownerdetailsurl" value="<?php echo esc_attr($currentapp['resourceownerdetailsurl']);?>"></td>
			</tr>
			...
			<tr>
				<td><strong><?php esc_html_e('Login Button:','oauth-client'); ?></strong></td>
				<td><div style="padding:5px;"></div><input type="checkbox" name="mo_oauth_show_on_login_page" value ="1" <?php if(isset($currentapp['show_on_login_page'])) { if($currentapp['show_on_login_page'] === 1 ) echo 'checked'; } ; ?>/><?php esc_html_e('Show on login page','oauth-client'); ?></td>
			</tr>
			<tr>
				<tr></tr>
				<td>&nbsp;</td>
		...
		<form id="form-common" name="form-common" method="post" action="admin.php?page=mo_oauth_settings">
		<?php wp_nonce_field('mo_oauth_attribute_mapping','mo_oauth_attribute_mapping_field'); ?>
		...
		<table class="mo_settings_table">
			<tr id="mo_oauth_email_attr_div">
				...
				<td><input class="mo_table_textbox" required="" placeholder="Username Attribute Name" type="text" id="mo_oauth_email_attr" name="mo_oauth_email_attr" value="<?php if(isset( $currentapp['email_attr']))echo esc_attr($currentapp['email_attr']);?>"></td>
			</tr>
			<tr>
				<td></td>
			<td>
...
function mo_oauth_apps_config_old() { 
	?>
		...
		<?php wp_nonce_field('mo_oauth_google','mo_oauth_google_field'); ?>
					...
					<table class="mo_settings_table">
						...
						<tr>
							...
							<td><input class="mo_table_textbox" required class="textbox"
								...
								value="<?php echo esc_attr(get_option('mo_oauth_google_client_id')); ?>" /></td>
						</tr>
						...
						<tr>
							...
							<td><input class="mo_table_textbox" required type="password"
								...
								value="<?php echo esc_attr(get_option('mo_oauth_google_client_secret')); ?>" /></td>
						</tr>
		...
		<form id="form-eveonline" name="form-eveonline" method="post"
			action="">
			<?php wp_nonce_field('mo_oauth_eveonline','mo_oauth_eveonline_field'); ?>
					...
					<table class="mo_settings_table">
						...
						<tr>
							...
							<td><input class="mo_table_textbox" required type="text"
								...
								value="<?php echo esc_attr(get_option('mo_oauth_eveonline_client_id')); ?>" /></td>
						</tr>

						<tr>
							<td><strong><font color="#FF0000">*</font>Client Secret:</strong></td>
							<td><input class="mo_table_textbox" type="password" required
								placeholder="Click on Help to know more"
								name="mo_oauth_eveonline_client_secret"
								value="<?php echo esc_attr(get_option('mo_oauth_eveonline_client_secret')); ?>" pattern="[a-zA-Z0-9\s]+" title="Please do not add any special characters." /></td>
						</tr>
						<tr>
						...
						<tr>
							<td colspan="2" id="eve_instru" hidden>
								...
								<ol>
									...
									<li>At EVE Online, set Redirect URL as <b><?php echo esc_url(site_url());?></b></li>
		...
		<?php wp_nonce_field('mo_oauth_facebook','mo_oauth_facebook_field'); ?>
					...
					<table class="mo_settings_table">
						...
						<tr>
							...
							<td><input class="mo_table_textbox" required class="textbox"
								...
								value="<?php echo esc_attr(get_option('mo_oauth_facebook_client_id')); ?>" /></td>
						</tr>
						...
						<tr>
							...
							<td><input class="mo_table_textbox" required type="text"
								...
								value="<?php echo esc_attr(get_option('mo_oauth_facebook_client_secret')); ?>" /></td>
						</tr>
							...
							<td colspan="2" id="facebook_instru" hidden>
								...
								<ol>
									<li>Go to Facebook developers console <a
										...
										rel="noopener" target="_blank">https://developers.facebook.com/apps/</a>.
									</li>
									...
									<li>Under <b>Client OAuth Settings</b>, enter <b><?php echo esc_url(site_url());?></b> in Valid OAuth redirect URIs and click <b>Save Changes</b>.</li>
...
<div id="mo_eve_online_config">
	...
	<form id="mo_eve_save_allowed" name="mo_eve_save_allowed" method="post"
		action="">
		<?php wp_nonce_field('mo_eve_save_allowed','mo_eve_save_allowed_field'); ?>
			...
			<table class="mo_settings_table">
				<tr>
					...
					<td><input class="mo_eve_table_textbox"
						...
						value="<?php echo esc_attr(get_option('mo_eve_allowed_corps'));?>" /></td>
				</tr>
				...
				<tr>
					...
					<td><input class="mo_eve_table_textbox"
						...
						value="<?php echo esc_attr(get_option('mo_eve_allowed_alliances'));?>" /></td>
				</tr>
				...
				<tr>
					...
					<td><input class="mo_eve_table_textbox"
						...
						value="<?php echo esc_attr(get_option('mo_eve_allowed_char_name'));?>" /></td>
				</tr>
				<tr>
					<td class="col1">&nbsp;</td>
...
?>
	<div class="mo_support_layout">
		<div>
			<h3>Contact Us</h3>
			<p>Need any help? Couldn't find an answer in <a href="https://faq.miniorange.com/kb/oauth-openid-connect" rel="noopener" target="_blank">FAQ</a>?<br>Just send us a query so we can help you.</p>
			<form method="post" action="">
			<?php wp_nonce_field('mo_oauth_contact_us_query_option','mo_oauth_contact_us_query_option_field'); ?>
				...
				<table class="mo_settings_table">
					...
					<tr>
						<td><input type="tel" id="contact_us_phone" pattern="[\+]\d{11,14}|[\+]\d{1,4}[\s]\d{9,10}" placeholder="Enter phone here" class="mo_table_textbox" name="mo_oauth_contact_us_phone" value="<?php $phone_no = get_option('mo_oauth_admin_phone'); if( $phone_no != "false" ) echo esc_attr($phone_no); ?>"></td>
					</tr>
					<tr>
...
{
	mo_oauth_hjsguh_kiishuyauh878gs($ejhi);								
}

function mo_oauth_jhuyn_jgsukaj($temp_var)
{
	mo_oauth_jkhuiysuayhbw($temp_var);
}
	...
	echo'<h2>Login Transactions Report</h2>
			<div class="mo_oauth_client_small_layout hidden">	
				<div style="float:right;margin-top:10px">
					<input type="submit" '.esc_attr($disabled).' name="printcsv" style="width:100px;" value="Print PDF" class="button button-success button-large">
					<input type="submit" '.esc_attr($disabled).' name="printpdf" style="width:100px;" value="Print CSV" class="button button-success button-large">
				</div>
				<h3>Advanced Report</h3>
				
				<form id="mo_oauth_client_advanced_reports" method="post" action="">
				';	wp_nonce_field('mo_oauth_client_advanced_reports','mo_oauth_client_advanced_reports_field');

				echo '<input type="hidden" name="option" value="mo_oauth_client_advanced_reports">
					<table style="width:100%">
					<tr>
					<td width="33%">WordPress Username : <input class="mo_oauth_client_table_textbox" type="text" '.esc_attr($disabled).' name="username" required="" placeholder="Search by username" value=""></td>
					<td width="33%">IP Address :<input class="mo_oauth_client_table_textbox" type="text" '.esc_attr($disabled).' name="ip" required="" placeholder="Search by IP" value=""></td>
					<td width="33%">Status : <select '.esc_attr($disabled).' name="status" style="width:100%;">
					...
					<tr><td><br></td></tr>
					<tr>
					<td width="33%">User Action : <select '.esc_attr($disabled).' name="action" style="width:100%;">
						  <option value="login" selected="">User Login</option>
						  <option value="register">User Registeration</option>
						</select>
					</td>
					<td width="33%">From Date : <input '.esc_attr($disabled).' class="mo_oauth_client_table_textbox" type="date"  name="fromdate"></td>
					<td width="33%">To Date :<input '.esc_attr($disabled).' class="mo_oauth_client_table_textbox" type="date"  name="todate"></td>
					</tr>
					</table>
					<br><input type="submit" '.esc_attr($disabled).' name="Search" style="width:100px;" value="Search" class="button button-primary button-large">
				</form>
				<br>
			</div>
