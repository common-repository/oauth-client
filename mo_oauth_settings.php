...
define( 'MO_OAUTH_CSS_JS_VERSION', '1.11.5' );
...
class mo_oauth {
	...
	function miniorange_oauth_save_settings(){
		...
		if( current_user_can( 'manage_options' ) && isset( $_POST['option'] )) {
			...
			if ( $_POST['option'] == "mo_oauth_client_mo_server_message" && wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['mo_oauth_client_mo_server_message_field'] ) ), 'mo_oauth_client_mo_server_message' )) {
					update_option( 'mo_oauth_client_show_mo_server_message', 1 );
					return;
			}
			
			if ( isset( $_POST['option'] ) && $_POST['option'] == "change_miniorange" && wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['change_miniorange_field'] ) ), 'change_miniorange' )) {
				$this->mo_oauth_deactivate();
				return;
			}
				
			if( $_POST['option'] == "mo_oauth_register_customer" && wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['mo_oauth_register_customer_field'] ) ), 'mo_oauth_register_customer' )) {
				$email = '';
				$phone = '';
				$password = '';
				$confirmPassword = '';
...
//				}
				...
				if( strcmp( $password, $confirmPassword) == 0 ) {
					...
					if( strcasecmp( $content['status'], 'CUSTOMER_NOT_FOUND') == 0 ){
						...
						if(strcasecmp($response['status'], 'SUCCESS') == 0) {
                            ...
                            wp_redirect( admin_url( '/admin.php?page=mo_oauth_settings&tab=login' ), 301 );
           					exit;
                        } else {
			...
			if( isset( $_POST['option'] ) and $_POST['option'] == "mo_oauth_client_goto_login" && wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['mmo_oauth_client_goto_login_field'] ) ), 'mo_oauth_client_goto_login' )) {
            	delete_option( 'new_registration' );
            	update_option( 'verify_customer', 'true' );
            } else if($_POST['option'] == "mo_oauth_verify_customer"  && wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['mo_oauth_verify_customer_field'] ) ), 'mo_oauth_verify_customer' )) {	//register the admin to miniOrange
//				if( mo_oauth_is_curl_installed() == 0 ) {
//					return $this->mo_oauth_show_curl_error();
//				}
			...
			else if( $_POST['option'] == "mo_eve_save_api_key"  && wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['mo_eve_save_api_key_field'] ) ), 'mo_eve_save_api_key' )){
//				if( mo_oauth_is_curl_installed() == 0 ) {
//					return $this->mo_oauth_show_curl_error();
//				}
				//validation and sanitization
			...
			else if( $_POST['option'] == "mo_eve_save_allowed" && wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['mo_eve_save_allowed_field'] ) ), 'mo_eve_save_allowed' )){
//				if( mo_oauth_is_curl_installed() == 0 ) {
//					return $this->mo_oauth_show_curl_error();
//				}
				//sanitization of corporations and alliance fields
				$corps = sanitize_text_field( stripslashes($_POST['mo_eve_allowed_corps'] ));
				$alliances = sanitize_text_field(stripslashes( $_POST['mo_eve_allowed_alliances'] ));
				$charName = sanitize_text_field(stripslashes( $_POST['mo_eve_allowed_char_name'] ));
			...
			else if( isset($_POST['option']) && $_POST['option'] == "mo_oauth_add_app" && wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['mo_oauth_add_app_field'] ) ), 'mo_oauth_add_app' )) {
				$scope = '';
				$clientid = '';
				$clientsecret = '';
				if($this->mo_oauth_check_empty_or_null($_POST['mo_oauth_client_id']) || $this->mo_oauth_check_empty_or_null($_POST['mo_oauth_client_secret'])) {
					...
					update_option('mo_oauth_client_disable_authorization_header',isset( $_POST['disable_authorization_header']) ? sanitize_text_field($_POST['disable_authorization_header']) : 0);
			...
			else if(  $_POST['option'] == "mo_oauth_app_customization" && wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['mo_oauth_app_customization_field'] ) ), 'mo_oauth_app_customization' )) {
			...
			else if( $_POST['option'] == "mo_oauth_attribute_mapping" && wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['mo_oauth_attribute_mapping_field'] ) ), 'mo_oauth_attribute_mapping' )) {
				$appname = stripslashes(sanitize_text_field( $_POST['mo_oauth_app_name'] ));
			...
			//submit google form
			else if( $_POST['option'] == "mo_oauth_google"  && wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['mo_oauth_google_field'] ) ), 'mo_oauth_google' ) ) {
//				if( mo_oauth_is_curl_installed() == 0 ) {
//					return $this->mo_oauth_show_curl_error();
//				}
				//validation and sanitization
				...
				if(mo_oauth_is_customer_registered()) {
					update_option( 'mo_oauth_google_enable', isset( $_POST['mo_oauth_google_enable']) ? sanitize_text_field($_POST['mo_oauth_google_enable']) : 0);
			...
			else if( $_POST['option'] == "mo_oauth_eveonline"  && wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['mo_oauth_eveonline_field'] ) ), 'mo_oauth_eveonline' )){
//				if( mo_oauth_is_curl_installed() == 0 ) {
//					return $this->mo_oauth_show_curl_error();
//				}
				//validation and sanitization
				...
				if(mo_oauth_is_customer_registered()) {
					update_option( 'mo_oauth_eveonline_enable', isset($_POST['mo_oauth_eveonline_enable']) ? sanitize_text_field($_POST['mo_oauth_eveonline_enable']) : 0);
					update_option( 'mo_oauth_eveonline_client_id', $clientid);
					update_option( 'mo_oauth_eveonline_client_secret', $clientsecret);
					if(get_option('mo_oauth_eveonline_client_id') && get_option('mo_oauth_eveonline_client_secret')) {
						$customer = new Customer();
			...
			// submit facebook app
			else if( $_POST['option'] == "mo_oauth_facebook" && wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['mo_oauth_facebook_field'] ) ), 'mo_oauth_facebook' )) {
//				if( mo_oauth_is_curl_installed() == 0 ) {
//					return $this->mo_oauth_show_curl_error();
//				}
				//validation and sanitization
				...
				if(mo_oauth_is_customer_registered()) {
					update_option( 'mo_oauth_facebook_enable', isset( $_POST['mo_oauth_facebook_enable']) ? sanitize_text_field($_POST['mo_oauth_facebook_enable']) : 0);
			...
			elseif( $_POST['option'] == "mo_oauth_contact_us_query_option" && wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['mo_oauth_contact_us_query_option_field'] ) ), 'mo_oauth_contact_us_query_option' )) {
//				if( mo_oauth_is_curl_installed() == 0 ) {
//					return $this->mo_oauth_show_curl_error();
//				}
				// Contact Us query
				$email = isset($_POST['mo_oauth_contact_us_email']) ? sanitize_text_field($_POST['mo_oauth_contact_us_email']) : '';
				$phone = isset($_POST['mo_oauth_contact_us_phone']) ? sanitize_text_field($_POST['mo_oauth_contact_us_phone']) : '';
				$query = isset($_POST['mo_oauth_contact_us_query']) ? sanitize_text_field($_POST['mo_oauth_contact_us_query']) : '';
			...
			else if($_POST['option'] == "mo_oauth_resend_otp_email"  && wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['mo_oauth_resend_otp_email_field'] ) ), 'mo_oauth_resend_otp_email' ) ) {
//				if( mo_oauth_is_curl_installed() == 0 ) {
//					return $this->mo_oauth_show_curl_error();
//				}
			...
			} else if ($_POST ['option'] == "mo_oauth_resend_otp_phone"  && wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['mo_oauth_resend_otp_phone_field'] ) ), 'mo_oauth_resend_otp_phone' ) ) {
...
//				}
					...
					if (strcasecmp($content ['status'], 'SUCCESS') == 0) {
						update_option('message', ' A one time passcode is sent to ' . esc_html($phone) . ' again. Please check if you got the otp and enter it here.');
			...
			} else if ($_POST ['option'] == 'mo_oauth_forgot_password_form_option'  && wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['mo_oauth_forgot_password_form_option_field'] ) ), 'mo_oauth_forgot_password_form_option' ) ) {
...
//					}
					...
					if (strcasecmp($content ['status'], 'SUCCESS') == 0) {
						update_option('message', 'Your password has been reset successfully. Please enter the new password sent to ' . esc_html($email) . '.');
						$this->mo_oauth_show_success_message();
				}
			} else if( $_POST['option'] == "mo_oauth_change_email" && wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['mo_oauth_change_email_field'] ) ), 'mo_oauth_change_email' ) ) {
				//Adding back button
				update_option('verify_customer', '');
				update_option('mo_oauth_registration_status','');
				update_option('new_registration','true');
			} else if( $_POST['option'] == "mo_oauth_register_with_phone_option" && wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['mo_oauth_register_with_phone_option_field'] ) ), 'mo_oauth_register_with_phone_option' )  ) {
//				if(!mo_oauth_is_curl_installed()) {
//					return $this->mo_oauth_show_curl_error();
//				}
				$phone = sanitize_text_field($_POST['phone']);
	...
	function create_customer(){
		$customer = new Customer();
		$customerKey = json_decode( $this->create_customer(), true );
...
//		}
	...
	function mo_oauth_shortcode_login(){
		if(mo_oauth_hbca_xyake() || !mo_oauth_is_customer_registered()) {
			return '<div style="text-align: center;border: 1px solid;margin: 5px;padding-top: 25px;"><p>This feature is supported only in standard and higher versions.</p>
	...
	function mo_oauth_my_show_extra_profile_fields($user) {
		...
		<table class="form-table">
			<tr>
				...
				<td>
					<input type="text" id="characterName" disabled="true" value="<?php echo esc_attr(get_user_meta( $user->ID, 'user_eveonline_character_name', true )); ?>" class="regular-text" /><br />
				</td>
				<td rowspan="3"><?php mo_oauth_avatar_manager_get_custom_avatar( $user->ID, '128' ); ?></td>
			</tr>
			<tr>
				<th><label for="corporation">Corporation Name</label></th>
				<td>
					<input type="text" id="corporation" disabled="true" value="<?php echo esc_attr(get_user_meta( $user->ID, 'user_eveonline_corporation_name', true )); ?>" class="regular-text" /><br />
				</td>
			</tr>
			<tr>
				<th><label for="alliance">Alliance Name</label></th>
				<td>
					<input type="text" id="alliance" disabled="true" value="<?php echo esc_attr(get_user_meta( $user->ID, 'user_eveonline_alliance_name', true )); ?>" class="regular-text" /><br />
				</td>
			</tr>
		</table>
	<?php
